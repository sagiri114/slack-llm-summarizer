...
输出语言由 `.env` 中的 `SUMMARY_LANGUAGE` 控制：
...
现在 `/summary` 会要求 LLM 返回更详细的结构，并在 Slack 中展示：
...
## Detailed Output and Commands

### Output language

Use `.env` to choose Chinese or Japanese:
...
The detailed output applies to all three Slack commands:

- `/summary`: conclusion, details, impact, next steps, evidence, confidence
- `/ask`: answer, key findings, reasoning, next steps, unknowns, evidence, confidence
- `/todo`: task, details, acceptance criteria, blockers/risks, owner, deadline, status, evidence, confidence

For longer answers, increase:
...
/ask 最近の demo の進捗は？
/ask 最近 demo 的风险是什么？
/ask provider:gemini 7d 誰が backend API を担当していますか？

/todo
/todo 24h
/todo 7d provider:openai
```

App mention also works in channels where the bot is invited:
...
```bash
slack-llm-summarizer
python -m slack_llm_summarizer.app
make run
```

### Dry-run summary without Slack

```bash
slack-summary-dry-run --input samples/messages.json
slack-summary-dry-run --input samples/messages.json --provider deepseek
python -m slack_llm_summarizer.cli --input samples/messages.json --hours 24
```
...
```bash
ssh chen_yiran@172.21.152.83
source ~/work/env.sh
cd ~/work/llm-tutorial
source .venv/bin/activate
cd slack-llm-summarizer
git pull --ff-only
pip install -e ".[dev]"
...
source ~/work/env.sh
cd ~/work/llm-tutorial
~/work/start_docker.sh
```

Inside the Docker shell:

```bash
cd ~/work/llm-tutorial/slack-llm-summarizer
source .venv/bin/activate
pip install -e ".[dev]"
python -m slack_llm_summarizer.check_provider
slack-llm-summarizer
